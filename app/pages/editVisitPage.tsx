...
export default function EditVisitPage() {
  ...
  const [originalVisitId, setOriginalVisitId] = useState<string>("");
  ...
  useEffect(() => {
    const fetchVisitData = async () => {
      if (searchParams.visitId) {
        setOriginalVisitId(searchParams.visitId as string);
        try {
          const visits = await loadVisits();
          const visit = visits.find(
            (v) =>
              v.id === searchParams.visitId ||
              v.dateTime === searchParams.visitId,
          );
          if (visit) {
            setFormData(visit);
          } else {
  ...
  const validateForm = async () => {
    ...
    if (!newErrors.dateTime && !newErrors.location) {
      try {
        await updateVisit(originalVisitId, formData);
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              await deleteVisit(originalVisitId);
              Alert.alert("Success", "Visit record deleted successfully!", [
                {
                  text: "OK",
                  onPress: () => router.back(),
                },
              ...
              Alert.alert(
                "Error",
                "Failed to delete visit record. Please try again.",
              );
            }
          },
        },
  ...
  return (
    <ScrollView contentContainerStyle={styles.mainContainer}>
      <View style={styles.container}>
        <View>
          <Text style={styles.recordTitle}>Edit Past Visit Record</Text>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <TouchableOpacity onPress={deleteForm}>
